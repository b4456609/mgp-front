...
function draw(graph, dispatch) {
  ...
  let lineData = [].concat(graph.serviceWithEndpointPair);
  if (graph.scenarioEndpointPair) {
    lineData = lineData.concat(graph.scenarioEndpointPair);
  }
  var servicesDep = svg
    .selectAll("line")
    .data(lineData)
  ...
  simulation
    .nodes(graph.nodes)
    .on("tick", ticked);
  let gr = [].concat(graph.providerEndpointWithConsumerPair);
  if (graph.serviceWithEndpointPair) {
    gr = gr.concat(graph.serviceWithEndpointPair)
  }
  if (graph.scenarioEndpointPair) {
    gr = gr.concat(graph.scenarioEndpointPair);
  }
